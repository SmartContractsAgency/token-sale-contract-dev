...
contract ASXContribution is Ownable, DSMath, TokenController{
    ...
    * @param _thresholdCoefficient - used to calculate the % change in threshold from Rone round to the next, WAD formatted (10**18 is 100%)
    ...
    */
    function initialize(
        ...
        require(_thresholdCoefficient >= 10**18);                                       // require that the threshold coefficient is greater than (or equal to) 100%
    ...
    */
    function initializeRound( uint _roundStart, uint _roundEnd) onlyOwner returns (bool success) {
        assert(initialized == true);                                        // assert that the ASXContribution contract has already been initialized (only relevant for Round 0)
    ...
    */
    function calcAvail() private returns (uint128 avail) {
        ...
        uint128 totalAllocation = wmul(cumulativeRoundPercent, cast(initSupply));   // calculate the maximum possible available ASX in the current round
    ...
    */
    function calcThreshold() private returns (uint128 threshold) {
        ...
        Round storage round = rounds[roundCount];                               // current round info (round.avail is set at this point)
        if (roundCount == 0) {
            ...
            uint prevCount = currentRound();
            Round storage prevRound = rounds[prevCount];                        // get the previous round information
            uint128 startPrice = wmul(thresholdCoefficient, prevRound.price);   // calculate the new starting price by multiplying the old price by the thresholdCoefficient
            threshold = wmul(startPrice, round.avail);                          // current threshold value is the new starting price multiplied by the available amount of ASX
            return threshold;
        }
    }
    ...
    function calcCap() private returns (uint128 cap) {
        ...
        Round storage round = rounds[roundCount];                               // current round info (round.avail is set at this point)
        if (roundCount == 0) {
            cap = wmul(currentRoundPercent, initMaxTarget);                     // round 0 cap depends only on the the initial max target and the round percentage
            return cap;
        } else {
            uint prevCount = currentRound();
            Round storage prevRound = rounds[prevCount];                        // get the previous round information
            uint128 maxPrice = wmul(capCoefficient, prevRound.price);           // calculate the current round max price by multiplying the old price by the capCoefficient
    ...
    */
    function contribute() payable returns (bool success) {
        ...
        if(round.totalContrib >= round.cap){                                                            // check for "round.totalContrib >= round.cap" end condition
            ...
            if(roundCount == 3){                                                                        // check for end of contribution at any round end, if it is the final round ending then fire contribution end
                contributionEnd();
            }
        }
    ...
    */
    function contributionEnd() private returns (bool success) {
        ...
        ContributionEnd(currentBlock, totalContributions, totalASXDistributed); // log the contribution end event
    ...
    */
    function collectFunds() onlyOwner returns (bool success) {
        assert(address(ASX.controller) == postContribController);   // assert that the postContribController contract is the controller of the ArtstockExchangeToken contract
                                                                    // ASX.controller can only be set to the postContribController during contributionEnd()
                                                                    // fund collection is only available after contribution period finalization for two reasons:
                                                                    // i) to guarantee ETH recycling is impossible during the contribution period, and
                                                                    // ii) to guarantee that Artstock controlled ASX tokens are always subject to the postContribController contract locking rules
    ...
    * @return (Round0, Round1, Round2) - returns any array for each round of the contribution
    */
    function getContributionInfo(address _participant) returns (uint128[10] Round0, uint128[10] Round1, uint128[10] Round2) {
